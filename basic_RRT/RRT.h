...
namespace RRT
{
	...
	{
		basic_RRT() = default;
		basic_RRT(const VectorXf xi, const VectorXf xg, const int m = 1000, const float s = 15) :step(s), max_size(m)
		{
		...
		basic_RRT(const Vehicle::Node &xi, const Vehicle::Node &xg, const int m = 1000, const float s = 15) :step(s), max_size(m)
		{
			root = RRT_Node(xi, 0, 0, m + 1);
			dest = RRT_Node(xg, m + 1, m + 1, m + 1);
			tree.reserve(m + 1);
			tree.emplace_back(root);
		}
