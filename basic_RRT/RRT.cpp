...
void RRT::basic_RRT::rand_select(Vehicle::Node *rand_node, const int &iter)
{
	...
	if (Vehicle::XMIN < dest._node()->x < (1 / Vehicle::KMAX) ? Vehicle::XMAX : dest._node()->x)
		rand_x = std::uniform_real_distribution<float>(Vehicle::XMIN, dest._node()->x < (1 / Vehicle::KMAX) ? Vehicle::XMAX : dest._node()->x);
	else
		rand_x = std::uniform_real_distribution<float>(dest._node()->x < (1 / Vehicle::KMAX) ? Vehicle::XMAX : dest._node()->x, Vehicle::XMIN);
...
std::string RRT::basic_RRT::grow(Vehicle::Node *new_node, Collision::collision *collimap)
{
	...
	if (is_new_effect(new_node))
	{
		...
		{
			if (!collimap->iscollision(near_node._node(), dest._node()))
			{
